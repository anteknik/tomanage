<?php if ($conversation) {
    ...
    foreach ($conversation as $value):
      ...
      "/>
                ...
                <span class="label label-info"><?php if (isset($value->sender_u)) {
                        echo $value->sender_u;
                    } else {
                        echo $value->sender_c;
                    } ?></span>
                <span
                    class="label"><?php echo date($core_settings->date_format . ' ' . $core_settings->date_time_format, $unix); ?></span>
                <?php if ($id == $value->id && $value->status != "deleted") { ?>
                    <a href="<?= base_url() ?>cmessages/delete/<?= $value->id; ?>" rel=""
                       class="confirm btn btn-mini pull-right ajax-silent btn-danger btn-loader btn-loader-red"><?= $this->lang->line('application_delete'); ?></a>
                <?php } ?>
            </div>
