<div id="main">
    <div id="options">
        <a href="<?= base_url() ?>quotation" class="btn"
           target="_blank"><?= $this->lang->line('application_view_quotation_form'); ?></a>
    ...
    <table class="data" id="quotations" rel="<?= base_url() ?>" cellspacing="0" cellpadding="0">
        ...
        <th class="hidden-phone"><?= $this->lang->line('application_company'); ?>
            / <?= $this->lang->line('application_name'); ?></th>
        ...
        <?php foreach ($quotations as $value): ?>
            ...
            <tr id="<?= $value->id; ?>">
                ...
                <td class="hidden-phone"><span class="label label-info"><?php if (!empty($value->company)) {
                            echo $value->company;
                        } else {
                            echo $value->fullname;
                        } ?></span></td>
                <td class="hidden-phone"><span class="label <?php if (isset($value->user->firstname)) {
        ...
        <div class="btn-group margintop5 pull-right nav-tabs2" data-toggle="buttons-radio">
            ...
            foreach ($submenu2 as $name => $value):?>
                ...
                if (!is_numeric(end($val_id))) {
                    ...
                    $num = count($val_id) - 2;
                    echo $val_id[$num];
                } ?>2" href="<?= site_url($value); ?>#custom_quotations_requests_filter"><?= $name ?></a>
            <?php endforeach; ?>
    ...
    <div class="visible-phone"><br/></div>
    <div class="table_head"><h6><i class="icon-list-alt"></i> <?= $this->lang->line('application_custom_quotations'); ?>
        </h6></div>
    <table class="data" id="custom_quotations_requests" rel="<?= base_url() ?>" cellspacing="0" cellpadding="0">
        ...
        <?php foreach ($custom_quotations as $value): ?>
            ...
            <tr id="<?= $value->id; ?>">
                ...
                <td class="hidden-phone"><?php if (isset($value->customquote->name)) {
                ...
                <td class="option btn-group">
                    <a class="btn btn-mini po" rel="popover" data-placement="left"
                       data-content="<a class='btn btn-danger po-delete ajax-silent' href='<?= base_url() ?>quotations/cdelete/<?= $value->id; ?>'><?= $this->lang->line('application_yes_im_sure'); ?></a> <button class='btn po-close'><?= $this->lang->line('application_no'); ?></button> <input type='hidden' name='td-id' class='id' value='<?= $value->id; ?>'>"
                       data-original-title="<b><?= $this->lang->line('application_really_delete'); ?></b>"><i
                            class="icon-trash"></i></a>
                    <a href="<?= base_url() ?>quotations/cupdate/<?= $value->id; ?>" class="btn btn-mini"
                       data-toggle="modal"><i class="icon-edit"></i></a>
                </td>
            </tr>
