...
<div class="container-fluid">
    ...
    <div class="row-fluid">
        <div id="sidebar" class="span2">
            ...
            <div class="sidebar-nav">
               ...
               <?php
                ...
                "/></div>
                    <div class="username">
                        <a href="<?= site_url("agent"); ?>" class="user"
                           data-toggle="modal"><?php echo character_limiter($this->user->firstname . " " . $this->user->lastname, 19); ?></a>
                        <br/>
                        <small><?= $this->user->title; ?></small>
                ...
                <ul class="nav dark nav-list clear hidden-phone hidden-tablet" id="menu">
                    ...
                    <?php foreach ($menu as $key => $value) { ?>
                        <li id="<?= strtolower($value->name); ?>"><a href="<?= site_url($value->link); ?>"><i
                                    class="icon-white <?= $value->icon; ?>"></i> <?php echo $this->lang->line('application_' . $value->link); ?>
                                <?php if (strtolower($value->name) == "messages" && $messages_new[0]->amount != "0") { ?>
                                    <span
                                        class="badge badge-important badge-new shadow pull-right"><?= $messages_new[0]->amount; ?></span><?php } ?>
                                <?php if (strtolower($value->name) == "quotations" && $quotations_new[0]->amount != "0") { ?>
                                    <span
                                        class="badge badge-important badge-new shadow pull-right"><?= $quotations_new[0]->amount; ?></span><?php } ?>
                                <?php if (strtolower($value->name) == "tickets" && $tickets_new[0]->amount != "0") { ?>
                                    <span
                                        class="badge badge-important badge-new shadow pull-right"><?= $tickets_new[0]->amount; ?></span><?php } ?>
                            </a></li>
                    <?php } ?>
                </ul>
            </div>
            ...
            foreach ($widgets as $key => $val) {
                ...
                if ($sticky && $val->link == "quickaccess") { ?>
                    <div class="widget hidden-phone hidden-tablet">
                        ...
                        <?php foreach ($sticky as $value): ?>
                            <small><a
                                    href="<?= base_url() ?>projects/view/<?= $value->id; ?>"><?= character_limiter($value->name, 15); ?></a>
                            </small>
                            <span class="label pull-right tt <?php if (empty($value->tracking)) {
                                echo ' label-light " title="' . $this->lang->line("application_start_timer") . '" ';
                            } else {
                                echo 'label-info" title="' . $this->lang->line("application_stop_timer") . '" ';
                            } ?>" rel="tooltip" data-placement="right"><a
                                    href="<?= base_url() ?>projects/tracking/<?= $value->id; ?>"
                                    id="<?= $value->id; ?>"><i class="icon-time icon-white"></i></a></span>
                            <div class="progress progress-striped active <?php if ($value->progress == "100") {
               ...
               <?php
                                    ...
                                    ?>
                "/> <?php echo $value->firstname . " " . $value->lastname; ?> <span class="pull-right"><img
                                            src="<?= base_url() ?>assets/blackline/img/<?php if ($value->last_active + (15 * 60) > time()) {
                                                echo "online";
                                            } else {
                                                echo "away";
                                            } ?>.png"/></span>
        ...
        <ul class="span10 breadcrumb">
            <li class="hidden-phone"><a href="<?= site_url(); ?>"><i class="icon icon-home"
                                                                     style="opacity: 0.7;"></i></a> <span
                    class="divider hidden-phone">></i></span></li>
            <li><a href="<?= site_url($act_uri); ?>"><?= ucwords($this->lang->line('application_' . $act_uri)); ?></a>
            </li>
            <?php if ($act_uri_submenu != $act_uri && $act_uri_submenu != "") { ?>
                <li class="active"><span
                    class="divider">></span> <?= $this->lang->line('application_' . $act_uri_submenu); ?></li><?php } ?>
            <li class="pull-right"><a href="<?= site_url("logout"); ?>" class="btn btn-mini"><i
            ...
            <li class="pull-right">
                ...
                <div class="btn-group">
                    <a class="btn btn-mini dropdown-toggle" data-toggle="dropdown" href="#">
                        ...
                        <img
                            src="<?= base_url() ?>assets/blackline/img/<?php if ($this->input->cookie('language') != "") {
                    ...
                    <ul class="dropdown-menu pull-right" role="menu" aria-labelledby="dLabel">
                        <?php if ($handle = opendir('application/language/')) {
                            ...
                            while (false !== ($entry = readdir($handle))) {
                                if ($entry != "." && $entry != "..") {
                                    ?>
                                    <li><a href="<?= base_url() ?>agent/language/<?= $entry; ?>"><img
                                            src="<?= base_url() ?>assets/blackline/img/<?= $entry; ?>.png"
                                            class="language-img"><?= ucwords($entry); ?></a></li><?php
                                }
                            }
...
<?php if ($this->session->flashdata('message')) {
    ...
    <div class="notification notification-<?= $exp[0] ?>">
        <div><span><?= $exp[1] ?></span></div>
    </div>
<?php } ?>
...
<script type="text/javascript" charset="utf-8">
    $(document).ready(function () {
        ...
        <?php if($act_uri_submenu != "0"){ ?>$(".submenu li a#<?php echo $act_uri_submenu; ?>").parent().addClass("active");
        <?php } ?>
