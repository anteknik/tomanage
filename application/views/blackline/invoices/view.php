<div id="main">
    <div id="options">
        <a href="<?= base_url() ?>invoices/update/<?= $invoice->id; ?>/view" class="btn" data-toggle="modal"><i
                class="icon-edit"></i> <?= $this->lang->line('application_edit_invoice'); ?></a>
        <a href="<?= base_url() ?>invoices/item/<?= $invoice->id; ?>" class="btn" data-toggle="modal"><i
                class="icon-plus-sign"></i> <?= $this->lang->line('application_add_item'); ?></a>

        <div class="visible-phone"><br/></div>
        <a href="<?= base_url() ?>invoices/preview/<?= $invoice->id; ?>" class="btn"><i
                class="icon-file"></i> <?= $this->lang->line('application_preview'); ?></a>
        <?php if ($invoice->status != "Paid" && isset($invoice->company->name)) { ?><a
            href="<?= base_url() ?>invoices/sendinvoice/<?= $invoice->id; ?>" class="btn"><i
                class="icon-envelope"></i> <?= $this->lang->line('application_send_invoice_to_client'); ?></a><?php } ?>
        ...
        <div class="span12 marginbottom20">
            ...
            <div class="subcont">
                <ul class="details span6">
                    <li><span><?= $this->lang->line('application_invoice_id'); ?>:</span> <?= $invoice->reference; ?>
                    </li>
                    <li class="<?= $invoice->status; ?>"><span><?= $this->lang->line('application_status'); ?>:</span>
                        <a class="label <?php $unix = human_to_unix($invoice->sent_date . ' 00:00');
                    ...
                    <li><span><?= $this->lang->line('application_issue_date'); ?>
                            :</span> <?php $unix = human_to_unix($invoice->issue_date . ' 00:00');
                        echo date($core_settings->date_format, $unix); ?></li>
                    <li><span><?= $this->lang->line('application_due_date'); ?>:</span> <a
                            class="label <?php if ($invoice->status == "Paid") {
                                echo 'label-success';
                            }
                ...
                <ul class="details span6">
                    <?php if (isset($invoice->company->name)) { ?>
                        <li><span><?= $this->lang->line('application_company'); ?>:</span> <a
                                href="<?= base_url() ?>clients/view/<?= $invoice->company->id; ?>"
                                class="label label-info"><?= $invoice->company->name; ?></a></li>
                        <li><span><?= $this->lang->line('application_contact'); ?>
                                :</span> <?php if (isset($invoice->company->client->firstname)) { ?><?= $invoice->company->client->firstname; ?> <?= $invoice->company->client->lastname; ?> <?php } else {
                                echo "-";
                            } ?></li>
                        <li><span><?= $this->lang->line('application_street'); ?>
                                :</span> <?= $invoice->company->address; ?></li>
                        <li><span><?= $this->lang->line('application_city'); ?>
                                :</span> <?= $invoice->company->zipcode; ?> <?= $invoice->company->city; ?></li>
                    <?php } else { ?>
    ...
    <div class="row">
        <div class="table_head"><h6><i class="icon-list-alt"></i><?= $this->lang->line('application_invoice_items'); ?>
            </h6><a href="<?= base_url() ?>invoices/item/<?= $invoice->id; ?>" class="btn btn-mini pull-right"
                    data-toggle="modal"><i class="icon-plus-sign"></i> <?= $this->lang->line('application_add_item'); ?>
            </a></div>
            ...
            <?php foreach ($items as $value): ?>
                <tr id="<?= $value->id; ?>">
                    <td class="option btn-group">
                        <a href="<?= base_url() ?>invoices/item_update/<?= $invoice->invoice_has_items[$i]->id; ?>"
                           class="btn btn-mini tt" title="<?= $this->lang->line('application_edit'); ?>"
                           data-toggle="modal"><i class="icon-edit"></i></a>
                        <a href="<?= base_url() ?>invoices/item_delete/<?= $invoice->invoice_has_items[$i]->id; ?>/<?= $invoice->id; ?>"
    ...
    <?php if ($core_settings->paypal == "1" && $sum != "0.00" && empty($invoice->paid_date)) { ?><br/>
        <form action="https://www.paypal.com/cgi-bin/webscr" method="post">
            ...
            <input type="submit" class="btn btn-primary pull-right" name="send"
                   value="<?= $this->lang->line('application_pay_via_paypal'); ?>">
