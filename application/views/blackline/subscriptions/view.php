<div id="main">
    <div id="options">
        <a href="<?= base_url() ?>subscriptions/update/<?= $subscription->id; ?>/view" class="btn"
           data-toggle="modal"><i class="icon-edit"></i> <?= $this->lang->line('application_edit_subscription'); ?></a>
        <a href="<?= base_url() ?>subscriptions/item/<?= $subscription->id; ?>" class="btn" data-toggle="modal"><i
                class="icon-plus-sign"></i> <?= $this->lang->line('application_add_item'); ?></a>
        ...
        <?php if ($subscription->end_date > $subscription->next_payment && date("Y-m-d") >= date('Y-m-d', strtotime('-3 day', strtotime($subscription->next_payment)))) { ?>
            <a href="<?= base_url() ?>subscriptions/create_invoice/<?= $subscription->id; ?>" class="btn"><i
                    class="icon-plus-sign"></i> <?= $this->lang->line('application_create_invoice'); ?></a>
        <?php } ?>
        <?php if ($subscription->status != "Paid" && isset($subscription->company->name)) { ?><a
            href="<?= base_url() ?>subscriptions/sendsubscription/<?= $subscription->id; ?>" class="btn"><i
                class="icon-envelope"></i> <?= $this->lang->line('application_send_subscription_to_client'); ?>
            </a><?php } ?>
        ...
        <div class="span12 marginbottom20">
            ...
            <div class="subcont">
                <ul class="details span6">
                    <li><span><?= $this->lang->line('application_subscription_id'); ?>
                            :</span> <?= $subscription->reference; ?></li>
                    <li class="<?= $subscription->status; ?>"><span><?= $this->lang->line('application_status'); ?>
                            :</span>
                        <a class="label <?php if ($subscription->status == 'Active') {
                            echo 'label-success';
                        } else {
                            echo 'label-important';
                        } ?>"><?php if ($subscription->end_date <= date('Y-m-d') && $subscription->status != "Inactive") {
                                echo $this->lang->line('application_ended');
                            } else {
                                echo $this->lang->line('application_' . $subscription->status);
                            } ?></a>
                        <?php if ($subscription->subscribed != "0") { ?>  <a
                            class="label label-success margin-left-5 tt"
                            title="<?php $unix = human_to_unix($subscription->subscribed . ' 00:00');
                            echo date($core_settings->date_format, $unix); ?>"><?= $this->lang->line('application_subscribed_via_paypal'); ?></a> <?php } ?>
                    </li>
                    <li><span><?= $this->lang->line('application_issue_date'); ?>
                            :</span> <?php $unix = human_to_unix($subscription->issue_date . ' 00:00');
                        echo date($core_settings->date_format, $unix); ?></li>
                    <li><span><?= $this->lang->line('application_end_date'); ?>:</span> <a
                            class="label <?php if ($subscription->end_date <= date('Y-m-d') && $subscription->status != "Inactive") {
                ...
                <ul class="details span6">
                    <?php if (isset($subscription->company->name)) { ?>
                        <li><span><?= $this->lang->line('application_company'); ?>:</span> <a
                                href="<?= base_url() ?>clients/view/<?= $subscription->company->id; ?>"
                                class="label label-info"><?= $subscription->company->name; ?></a></li>
                        <li><span><?= $this->lang->line('application_contact'); ?>
                                :</span> <?php if (isset($subscription->company->client->firstname)) { ?> <?= $subscription->company->client->firstname; ?> <?= $subscription->company->client->lastname; ?> <?php } else {
                                echo "-";
                            } ?></li>
                        <li><span><?= $this->lang->line('application_street'); ?>
                                :</span> <?= $subscription->company->address; ?></li>
                        <li><span><?= $this->lang->line('application_city'); ?>
                                :</span> <?= $subscription->company->zipcode; ?> <?= $subscription->company->city; ?>
                        </li>
                        <li><span><?= $this->lang->line('application_website'); ?>
                                :</span> <?= $subscription->company->website; ?></li>
    ...
    <div class="row">
        <div class="table_head"><h6><i
                    class="icon-list-alt"></i><?= $this->lang->line('application_subscription_items'); ?></h6><a
                href="<?= base_url() ?>subscriptions/item/<?= $subscription->id; ?>" class="btn btn-mini pull-right"
                data-toggle="modal"><i class="icon-plus-sign"></i> <?= $this->lang->line('application_add_item'); ?></a>
        </div>
        <table id="items" rel="<?= base_url() ?>" cellspacing="0" cellpadding="0">
            ...
            <?php foreach ($items as $value): ?>
                <tr id="<?= $value->id; ?>">
                    <td class="option btn-group">
                        <a href="<?= base_url() ?>subscriptions/item_update/<?= $subscription->subscription_has_items[$i]->id; ?>"
                           class="btn btn-mini tt" title="<?= $this->lang->line('application_edit'); ?>"
                           data-toggle="modal"><i class="icon-edit"></i></a>
                        <a href="<?= base_url() ?>subscriptions/item_delete/<?= $subscription->subscription_has_items[$i]->id; ?>/<?= $subscription->id; ?>"
                           class="btn btn-mini tt" title="<?= $this->lang->line('application_delete'); ?>"><i
                                class="icon-trash"></i>
                    </td>
                ...
                <?php $sum = $sum + $subscription->subscription_has_items[$i]->amount * $subscription->subscription_has_items[$i]->value;
                $i++; ?>
            ...
            <?php if ($core_settings->tax != "0") { ?>
                <tr>
                    <td colspan="5" align="right"><?= $this->lang->line('application_tax'); ?>
                        (<?= $core_settings->tax ?>%)
                    </td>
                    <td><?= $tax ?></td>
                </tr>
    ...
    <?php if ($core_settings->paypal == "1" && $sum != "0.00" && $subscription->subscribed == "0") { ?><br/>
        <form action="https://www.paypal.com/cgi-bin/webscr" method="post">
            ...
            <input type="hidden" name="item_name"
                   value="<?= $this->lang->line('application_subscription'); ?> #<?= $subscription->reference; ?>">
            ...
            <input type="hidden" name="cancel_return"
                   value="<?= base_url() ?>csubscriptions/view/<?= $subscription->id; ?>">
            ...
            <input class="btn btn-primary pull-right" type="submit"
                   value="<?= $this->lang->line('application_subscribe_via_paypal'); ?>" border="0" name="send">
        </form>
        <br clear="all">
    <?php } ?>
...
<div class="row">
    <div class="table_head"><h6><i
                class="icon-list-alt"></i><?= $this->lang->line('application_subscription'); ?> <?= $this->lang->line('application_invoices'); ?>
        </h6></div>
    <table class="data" id="invoices" rel="<?= base_url() ?>" cellspacing="0" cellpadding="0">
        ...
        <?php foreach ($subscription->invoices as $value): ?>
            ...
            <tr id="<?= $value->id; ?>">
                ...
                <td class="hidden-phone"><span class="label label-info"><?php if (!isset($value->company->name)) {
                            echo $this->lang->line('application_no_client_assigned');
                        } else {
                            echo $value->company->name;
                        } ?></span></td>
                <td><span><?php $unix = human_to_unix($value->issue_date . ' 00:00');
                ...
                <td><span class="label <?php if ($value->status == "Paid") {
                    ...
                    if ($value->due_date <= date('Y-m-d') && $value->status != "Paid") {
                        ...
                        echo date($core_settings->date_format, $unix); ?></span></td>
                <td><span class="label <?php $unix = human_to_unix($value->sent_date . ' 00:00');
                ...
                <td class="option btn-group">
                    ...
                    <a href="<?= base_url() ?>invoices/update/<?= $value->id; ?>" class="btn btn-mini"
                       data-toggle="modal"><i class="icon-edit"></i></a>
                </td>
            </tr>
