...
<div class="modal-content">
    <div class="modal-inner">
        ...
        <p>
            ...
            <input id="name" type="text" name="name" class="required span5 resetvalue" value="<?php if (isset($task)) {
                echo $task->name;
            } ?>"/>
        </p>
    ...
    <div class="modal-footer">
        <?php if (isset($task)) { ?>
            <a href="<?= base_url() ?>projects/tasks/<?= $task->project_id; ?>/delete/<?= $task->id; ?>"
               class="btn btn-danger pull-left"><?= $this->lang->line('application_delete'); ?></a>
        <?php } else { ?>
            <i class="icon-loader-black" style="display:none"></i>
            <input type="submit" id="send" name="send" class="btn btn-primary"
                   value="<?= $this->lang->line('application_save_and_add'); ?>"/>
        <?php } ?>
