<div id="main">
    <div id="options">
        <a href="<?= base_url() ?>projects/create" class="btn" data-toggle="modal"><i
                class="icon-plus-sign"></i> <?= $this->lang->line('application_create_new_project'); ?></a>
    ...
    <div class="table_head"><h6><i class="icon-briefcase"></i><?= $this->lang->line('application_projects'); ?></h6>
    </div>
    <table class="data" id="projects" rel="<?= base_url() ?>" cellspacing="0" cellpadding="0">
        ...
        <?php foreach ($project as $value):
            $workers = array();
            foreach ($value->project_has_workers as $worker) {
                array_push($workers, $worker->user_id);
            }
            if ($this->user->admin == "1" || in_array($this->user->id, $workers)) { ?>
                ...
                <tr id="<?= $value->id; ?>">
                    ...
                    <td class="hidden-phone"><span
                            class="label <?php if ($value->end <= date('Y-m-d') && $value->progress != 100) {
                    ...
                    <td class="option btn-group">
                        <a class="btn btn-mini po" rel="popover" data-placement="left"
                           data-content="<a class='btn btn-danger po-delete ajax-silent' href='<?= base_url() ?>projects/delete/<?= $value->id; ?>'><?= $this->lang->line('application_yes_im_sure'); ?></a> <button class='btn po-close'><?= $this->lang->line('application_no'); ?></button> <input type='hidden' name='td-id' class='id' value='<?= $value->id; ?>'>"
                           data-original-title="<b><?= $this->lang->line('application_really_delete'); ?></b>"><i
                                class="icon-trash"></i></a>
                        <a href="<?= base_url() ?>projects/update/<?= $value->id; ?>" class="btn btn-mini"
