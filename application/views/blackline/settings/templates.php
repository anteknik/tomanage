...
<div class="btn-group margintop5 nav-tabs normal-white-space" data-toggle="buttons-radio">
    <?php foreach ($template_files as $value) { ?>
        <a href="<?= base_url() ?>settings/templates/<?= $value; ?>"
           class="btn btn-mini <?php if ($value == $template) {
               echo "active";
           } ?>"><?= $this->lang->line('application_' . $value . '_email_template'); ?></a>
    <?php } ?>
</div>
...
<div class="table_head"><h6><i
            class="icon-list-alt"></i><?= $this->lang->line('application_' . $template . '_email_template'); ?></h6>
</div>
<?php
...
<table id="settings" cellspacing="0" cellpadding="0">
    <?php if (isset($settings->{$template . '_mail_subject'})) { ?>
        <tr>
            ...
            <td><input type="text" name="<?= $template; ?>_mail_subject" class="required no-margin span6"
                       value="<?= $settings->{$template . '_mail_subject'}; ?>"></td>
        </tr>
    <?php } ?>
    <tr>
        <td style="vertical-align:top"><?= $this->lang->line('application_mail_body'); ?> <br/>
            <a href="<?= base_url() ?>settings/settings_reset/email_<?= $template; ?>" class="btn btn-mini tt"
