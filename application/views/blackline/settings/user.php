<div id="main">
    ...
    <hr>
    <div class="table_head"><h6><i class="icon-user"></i><?= $this->lang->line('application_all_users'); ?></h6> <a
            href="<?= base_url() ?>settings/user_create" class="btn btn-mini pull-right" data-toggle="modal"><i
                class="icon-plus-sign"></i> <?= $this->lang->line('application_create_user'); ?></a></div>
        ...
        <?php foreach ($users as $user): ?>
            ...
            <tr id="<?= $user->id; ?>">
               ...
               <?php
                ...
                <td><?= $user->title; ?></td>
                <td><?= $user->email; ?></td>
                <td><span class="label label-<?php if ($user->status == "active") {
                        echo "success";
                    } else {
                        echo "important";
                    } ?>"><?= $this->lang->line('application_' . $user->status); ?></span></td>
                <td><span class="label label-<?php if ($user->admin == "1") {
                        echo "success";
                    } else {
                        echo "";
                    } ?>"><?php if ($user->admin) {
                ...
                <td><span><?php if (!empty($user->last_login)) {
                ...
                <td class="option btn-group">
                    <a class="btn btn-mini po" rel="popover" data-placement="left"
                       data-content="<a class='btn btn-danger po-delete ajax-silent' href='<?= base_url() ?>settings/user_delete/<?= $user->id; ?>'><?= $this->lang->line('application_yes_im_sure'); ?></a> <button class='btn po-close'><?= $this->lang->line('application_no'); ?></button> <input type='hidden' name='td-id' class='id' value='<?= $user->id; ?>'>"
                       data-original-title="<b><?= $this->lang->line('application_really_delete'); ?></b>"><i
                            class="icon-trash"></i></a>
                    <a href="<?= base_url() ?>settings/user_update/<?= $user->id; ?>" class="btn btn-mini"
